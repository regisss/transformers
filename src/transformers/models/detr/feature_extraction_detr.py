...
class DetrFeatureExtractor(FeatureExtractionMixin, ImageFeatureExtractionMixin):
    ...
    def post_process_semantic_segmentation(self, outputs, target_sizes: List[Tuple[int, int]] = None):
        ...
        np.save("circle_masks_classes", masks_classes.numpy())
        np.save("circle_masks_probs", masks_probs.numpy())

        # Semantic segmentation logits of shape (batch_size, num_classes, height, width)
        segmentation = torch.einsum("bqc, bqhw -> bchw", masks_classes, masks_probs)

        np.save("circle_segmentation", segmentation.numpy())
        ...
        if target_sizes is not None:
            ...
            for idx in range(batch_size):
                ...
                np.save(f"circle_resized_logits_{idx}", resized_logits.numpy())

                semantic_map = resized_logits[0].argmax(dim=0)
                np.save(f"circle_semantic_map_{idx}", semantic_map.numpy())

                semantic_segmentation.append(semantic_map)
        else:
            semantic_segmentation = segmentation.argmax(dim=1)
            np.save(f"circle_semantic_segmentation", semantic_segmentation.numpy())
